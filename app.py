...
class IntelliSearch:
    ...
    async def run(self):
        ...
        # Show detailed system status information (but don't block the interface)
        if self.system_status:
            ...
            if self.system_status['system_mode'] == 'full_semantic':
                st.success(f"🚀 **Full Semantic Mode Active** - All advanced features available: {', '.join(active_caps)}")
            elif self.system_status['system_mode'] == 'vector_only':
                st.info(f"📊 **Vector Search Mode** - Advanced search available: {', '.join(active_caps)}")
            else:
                st.warning(f"🔍 **{self.system_status['system_mode'].replace('_', ' ').title()}** - Available: {', '.join(active_caps)}")
